...
//"reqAdvising ---> " . $reqAdvising;
	...
	$sql="update miudb.advising_history a set a.advising_condition='$reqAdvising' where 
			(a.student_id='$sid') 
		and (a.semester= '$semester') 
		and (a.open_year= '$open_year')";
	echo $sql;
	mysql_query($sql);
	if(mysql_affected_rows()>=1) echo("<script>alert('Must information was successfully updated!'); </script>");
	else if ($rs_1[student_id] =="") echo ("<script>alert ('You should registrate Advising from Advising List!');</script>"); 
			...
			<tr align="center"><td colspan='3'>
				...
				&nbsp;&nbsp;Semester : 
		    		<Select name="semester" size="1">
			 			<option value="" <?if($semester=="") echo(" selected");?>>All</option>
						<option value="1" <?if($semester=="1") echo(" selected");?>>Spring</option>
						<option value="2" <?if($semester=="2") echo(" selected");?>>Summer</option>
						<option value="3" <?if($semester=="3") echo(" selected");?>>Fall</option>
						<option value="4" <?if($semester=="4") echo(" selected");?>>Winter</option>
					</Select>
...
<table align="center" cellspacing="1" cellpadding="3" class="datatable" bgcolor="#cdcdcd">
	<tr class="header">
		...
		<!--td align="center" width='40'>Semester</td-->
...
if ($res)
	while ($rs = mysql_fetch_array($res)) {	
		...
		//echo "<td width='40'>".getSemName($rs_1[semester])."</td>";
...
echo "<td width='80'><font color=".$font_color.">".getStatusName($rs[status])."</font></td>";*/
	...
	//$sql_1 ="select * from miudb.advising_history a, miudb.advising_reference r where (student_id = '$rs[student_id]') and (a.open_year = '$rs_1[open_year]')";
	$sql_1 ="select * from miudb.advising_history  where (student_id = '$rs[student_id]') and (semester = '$semester') ";
	...
	if($rs_1[advising_condition]==1 && $rs_1[open_year] == $open_year && $rs_1[semester] == $semester) echo "checked></td>";
	else echo ">";
	echo "<td width='50'><input bgcolor='gray' type='button' value='Detail' onclick=call_detail('".$rs[student_id]."','".$open_year."','".$semester."','".$school_year."') ".$use_button."></td>";
	echo "</tr>";
$cnt++;
}
...
function call_detail(student_id, open_year, semester,school_year) {
	var checkIdWin = window.open('advising_detail.php?student_id='+student_id+'&open_year='+open_year+'&semester='+semester+'&school_year='+school_year,'','width=870,height=700,scrollbars=yes,left=20,top=0,resizable=yes,location=no');
	checkIdWin.focus();
}
